...
int main(int argc, char *argv[])
{
     ...
     */
    int option = (int) atoi(argv[1]);
    switch (option) {
        case 1: {
            MPI_Allgather(sendbuf.data(), 2*NTASKS, MPI_INT, recvbuf.data(), 2*NTASKS, MPI_INT, MPI_COMM_WORLD);
            break;
        }
        case 2: {
            MPI_Scatter(sendbuf.data(), NTASKS/2, MPI_INT, recvbuf.data(), NTASKS/2, MPI_INT, 0, MPI_COMM_WORLD);
            break;
        }
        case 3: {
            int sendc[NTASKS] = {1,1,2,4};
            int recvcounts[NTASKS] = {1,1,2,4};
            int displs[NTASKS] = {0,1,2,4};
            MPI_Gatherv(sendbuf.data(), sendc[rank], MPI_INT, recvbuf.data(), recvcounts, displs, MPI_INT, 1, MPI_COMM_WORLD);
            break;
        }
        case 4: {
